            ...
            $.get('/gossip/getgossips/'+curPosition+'/'+nextPosition+'/',function(data){
                ...
                for (var j=1;j<=l;j++)
                    if (!gossips.in_array(obj[j-1].gossip_id.toString())) {
                        $("#column"+c).append('<div class="panel panel-default"><div class="panel-heading"><h3 class="panel-title" style="color:#0066FF;"><a href="/lecture/'+obj[j-1].lecture.id+'/">'+obj[j-1].lecture.professor_name+' 《'+obj[j-1].lecture.course_name+'》</a></h3></div><div class="panel-body"><h4 style="text-indent:2em;white-space:pre-wrap;line-height:30px;">'+obj[j-1].gossip_content+'</h4><h4 style="text-align:right;white-space:pre;"><small>'+obj[j-1].gossip_time+'      <a href="#" class="goss'+fetchtime+'" data-gosid="'+obj[j-1].gossip_id+'" data-super="'+(obj[j-1].have_supered?'1':'0')+'">'+(obj[j-1].have_supered?'取消':'赞')+'</a> (<span id="s'+obj[j-1].gossip_id+'">'+obj[j-1].gossip_super_number+'</span>)</small></h4></div></div>');
                        c=c%3+1;
